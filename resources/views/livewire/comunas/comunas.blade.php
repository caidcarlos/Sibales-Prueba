    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-2">
            ...
            <div class="flex justify-between">
                <div class="w-1/4">
                    <button wire:click="crear()" class="py-3 px-4 uppercase font-bold text-white rounded-lg bg-red-600 shadow-lg block md:inline-block my-8">Agregar Comuna</button>
                </div>
                <div class="w-1/2">
            ...
            @if($modal_create)
                @include('livewire.comunas.modal-create')
            @endif
            @if($modal_update)
                @include('livewire.comunas.modal-update')
            @endif
            @if($modal_confirm)
                @include('livewire.comunas.modal-confirm')
            @endif
            ...
            <table class="table-fixed w-full">
                ...
                <tbody>
                    @foreach ($comunas as $comuna)
                        <tr>
                            <td class="border px-4 py-2 text-center text-xl">{{$comuna->codigo}}</td>
                            <td class="border px-4 py-2 text-center text-xl">{{$comuna->nombre}}</td>
                            <td class="border px-4 py-2 text-center text-xl">{{$comuna->direccion}},
                                @foreach ($parroquias as $parroquia)
                                    @foreach ($municipios as $municipio)
                                        @if ($comuna->id_parroquia == $parroquia->id && $parroquia->id_municipio == $municipio->id)
                            ...
                            <td class="border px-4 py-2 flex justify-between">
                                <button wire:click="editar({{$comuna->id}})" 
                                ...
                                <button wire:click="confirmar({{$comuna->id}})"
            ...
            <div class="my-4"> {{ $comunas->links() }} </div>
        </div>
    </div>
</div>