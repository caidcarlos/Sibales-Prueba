    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-2">
            ...
            <div class="flex justify-between">
                <div class="w-1/4">
                    <button wire:click="crear()" class="py-3 px-4 uppercase font-bold text-white rounded-lg bg-red-600 shadow-lg block md:inline-block my-8">Agregar Municipio</button>
                </div>
                <div class="w-1/2">
            ...
            @if($modal_create)
                @include('livewire.municipios.modal-create')
            @endif
            @if($modal_update)
                @include('livewire.municipios.modal-update')
            @endif
            @if($modal_confirm)
                @include('livewire.municipios.modal-confirm')
            @endif
                    ...
                    @foreach ($municipios as $municipio)
                        <tr>
                            ...
                            <td class="border px-4 py-2 flex justify-between">
                                ...
                                <button wire:click="confirmar({{$municipio->id}})"
