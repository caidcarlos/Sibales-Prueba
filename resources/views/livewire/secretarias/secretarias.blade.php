    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-2">
            ...
            <div class="flex justify-between">
                <div class="w-1/4">
                    <button wire:click="crear()" class="py-3 px-4 uppercase font-bold text-white rounded-lg bg-red-600 hover:bg-red-500 shadow-lg block md:inline-block my-8">Agregar Secretaría</button>
                </div>
                <div class="w-1/2">
            ...
            @if($modalCreate)
                @include('livewire.secretarias.modal-create')
            @endif
            @if($modalUpdate)
                @include('livewire.secretarias.modal-update')
            @endif
            @if($modalConfirm)
                @include('livewire.secretarias.modal-confirm')
            @endif

            <table class="table-fixed w-full">
                <thead>
                    <tr class="bg-red-600 text-white uppercase text-xl py-4 my-10">
                        <th class="w-1/2">Secretaría</th>
                ...
                <tbody>
                    @foreach ($secretarias as $secretaria)
                        <tr>
                            <td class="border px-4 py-2 text-center text-xl">{{$secretaria->nombre}}</td>
                            <td class="border px-4 py-2 text-center text-xl">
                                @if ($secretaria->status==1)
                                    Activo                                    
                                @else
                                    Inactivo
                                @endif
                            ...
                            <td class="border px-4 py-2 flex justify-between">
                                <button wire:click="editar({{$secretaria->id}})" 
                                ...
                                <button wire:click="confirmar({{$secretaria->id}})"
